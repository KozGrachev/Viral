import { clearMisinfoAction, debunkMisinfoAction, moveAction, shareCardAction, discardCardAction, StartGameAction, AddPlayerAction } from '../redux/gameState/gameStateActions'
...
export const startGameEvent = (players: Player[]) => {
  dispatch(StartGameAction(players))
}

export const addPlayerEvent = (name: string) => {
  const color = 'blue'
  const room = 'test room'
  dispatch(AddPlayerAction(name, color, room))
}